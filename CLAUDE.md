...
- **Credentials** : `.env.test` + `.env.production` racine (non commités — `.env.template` commité
  ...
  Airflow locale, dashboard Render). `api/.env` existe mais n'est lu par aucun chemin de code
  actuel (Makefile lance `uvicorn` depuis la racine, `load_dotenv()` y trouve donc `.env.test`) —
...
- **Phase 6** ⏳ — Dashboard de suivi Streamlit (`dashboard/app.py`) : implémenté et testé en
  ...
  déjà en prod Neon ; `NULL` pour les transactions collectées avant ce changement). Déploiement
