...
- **PgSQL prod** (Neon, projet `fraud-detection-db`) : chaîne de connexion dans `DATABASE_URL`
  (valeur prod, cf. `airflow/.env.production`/`dashboard/.env.production`)
...
- **Credentials** : un fichier `.env.test`/`.env.production` par application (non commités —
  `api/.env.template`, `airflow/.env.template`, `dashboard/.env.template` commités documentent
  chacun leurs variables). Pas de fichier racine : chaque app ne connaît que le sous-ensemble de
  variables qu'elle lit réellement (9 pour l'API, 17 pour Airflow, 2 pour le dashboard) — le
  contenu de `api/.env.production`/`dashboard/.env.production` est directement ce qu'il faut
  copier dans le dashboard Render / les secrets Streamlit Cloud (ni l'un ni l'autre ne lit un
  fichier local). `api/.env` (sans suffixe) existe mais n'est lu par aucun chemin de code actuel
  — fichier vestigial, à ignorer.
...
| `MODEL_PATH` | chemin pkl | Modèle local en mode test — vide dans `api/.env.production` (ignoré si `MODEL_ENV≠test`) |
| `APP_ENV` | `test` \| `prod` | Mode des DAGs Airflow (`dags/config.py`) — bascule `DATABASE_URL` dans `airflow/.env.test`/`.env.production` (même nom des deux côtés, plus de `DATABASE_URL_PROD` séparée) |
| `MODEL_RELOAD_TOKEN` | secret partagé | Protège `POST /reload-model` (`api/app.py`) — même valeur dans `api/.env.*` et `airflow/.env.*` (`dags/tasks/train_model.py`), sinon 403 |

## DAGs Airflow (`dags/`)

Nommage aligné sur specs.md (§3.1/3.2/3.3) depuis le 2026-07-07 (avant : `fraud_detection_realtime`/
`fraud_detection_train_model`/`fraud_detection_daily_report`). Les 3 schedules sont des expressions
cron (`schedule=` accepte nativement une string cron sous Airflow 3.x) — plus de mélange
`timedelta(minutes=...)`/cron dérivé par f-string.

| specs.md | `dag_id` | Fichier | Variable de schedule | Défaut | Timetable résolue |
|----------|----------|---------|----------------------|--------|--------------------|
| §3.1 DAG ETL & Fraud Detection | `dag_etl_fraud_detection` | `dags/dag_realtime.py` | `DAG_ETL_FRAUD_DETECTION_CRON` | `* * * * *` | toutes les minutes |
| §3.3 DAG Model Training | `dag_train_model` | `dags/dag_train_model.py` | `DAG_TRAIN_CRON` | `0 * * * *` | toutes les heures |
| §3.2 DAG Fraud Report | `dag_report` | `dags/dag_daily_report.py` | `DAG_REPORT_CRON` | `0 0 * * *` | tous les jours à minuit |

Les 3 variables vivent dans `airflow/.env.test`/`.env.production` (mêmes clés des deux côtés, cf.
`airflow/.env.template`) et sont propagées par `airflow/start.sh` + `airflow/docker-compose.yml`.
`DAG_REPORT_CRON` a un rôle double dans `dag_daily_report.py` : il pilote à la fois le `schedule=`
du DAG et la fenêtre de lookback du rapport — cette dernière n'est **pas** une variable séparée,
elle est dérivée de l'expression cron elle-même via `croniter.get_prev()` (dépendance déjà présente
dans l'image Airflow), pour rester automatiquement synchronisée si le cron change.
...
- **Phase 4** ✅ — tests unitaires (`tests/`, `make test` ; ne couvrent que `src/`, les tests
  ...
  `dag_etl_fraud_detection` exécuté de bout en bout (fetch_trx → store_trx → fraud_detect →
...
- **Phase 6** ✅ — Dashboard de suivi Streamlit (`dashboard/app.py`) : implémenté et testé en
  ...
  `DATABASE_URL`/`FRAUD_API_URL` configurés directement dans Streamlit Cloud (contenu de
  `dashboard/.env.production`, jamais chargé par du code — Streamlit Cloud ne lit aucun fichier
  local, comme Render pour l'API).
