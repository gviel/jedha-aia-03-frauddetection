...
- **Credentials** : dans `api/.env` (non commité) — NE PAS commiter
...
| `DATABASE_URL_PROD` | URL PgSQL Neon | `.env` racine ; utilisée par `airflow/start.sh` quand `APP_ENV=prod` (DAGs 3.1/3.2 → Neon au lieu de fraud-db local) |
...
**Catégorielles** (LabelEncoder) : `gender, state, category, merchant, job`
...
config/models.yaml                   # 4 modèles + test_size + resampling
...
- **Phase 5** ⏳ — Déploiement prod (specs.md : MLFlow HF, API Render, PgSQL Neon, **Airflow
  ...
  - **Déploiement API sur Render** : pas commencé (aucun compte/service Render créé). Terrain
    préparé côté repo (`api/Dockerfile` + liste des env vars requises), voir
    `docs/travail/architecture_deployment.md`.
  - **Sync S3 des artefacts d'entraînement (DAG 3.3)** : specs.md demande de stocker sur
    `bucket-fraud-detection-gviel` les 4 fichiers `data/fraudTest.csv`, `work/fraudTest.csv`,
    `work/fraudTest_prepared.csv` et `work/client_trx_analysis.csv`. Trois des quatre ont été
    copiés manuellement (`aws s3 cp`, pas automatisé dans le DAG) ; seul
    `work/fraudTest_prepared.csv` reste non synchronisé. Le DAG 3.3 ne fait toujours aucun
    upload S3 lui-même (copie manuelle, pas de logique S3-à-S3 dans `dags/tasks/train_model.py`)
    — gap connu, différé (cf. mémoire Claude Code).
...
- **Phase 6** ⏳ — Dashboard de suivi Streamlit (`dashboard/app.py`) : implémenté et testé en
  ...
  panneau statut API/modèle via `/health` (étendu avec `model_name`/`api_version`). Déploiement
  prod (Streamlit Community Cloud) **non fait**, hors scope demandé pour l'instant.
